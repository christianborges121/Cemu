...
static const uint8_t GF2_8_LOG[256] = {
...
static const uint8_t GF2_8_EXP[510] = {
...
static const uint8_t GF2_8_INV[256] = {
...
inline void axpy(uint8_t* dst, const uint8_t* src, uint8_t u, int len)
{
	if (u == 0)
		return;
	if (u == 1)
	...
	{
		const uint8_t logU = GF2_8_LOG[u];
		for (int i = 0; i < len; ++i)
		{
			const uint8_t b = src[i];
			if (b != 0)
			{
				dst[i] ^= GF2_8_EXP[logU + GF2_8_LOG[b]];
			}
		}
	}
}
...
bool ReedSolomon::Encode(
	...
	int blockSize)
{
	if (!dataShards || !parityShards || dataCount <= 0 || parityCount <= 0 || blockSize <= 0)
		return false;

	if (dataCount > MAX_DATA_SHARDS || parityCount > MAX_PARITY_SHARDS)
		return false;

	// Build Cauchy generator matrix: P[j, i] = GF2_8_INV[(parityCount + i) ^ j]
	for (int j = 0; j < parityCount; ++j)
	{
		uint8_t* parityRow = parityShards[j];
		std::memset(parityRow, 0, static_cast<size_t>(blockSize));

		for (int i = 0; i < dataCount; ++i)
		{
			const uint8_t u = GF2_8_INV[(parityCount + i) ^ j];
			axpy(parityRow, dataShards[i], u, blockSize);
		}
	}
